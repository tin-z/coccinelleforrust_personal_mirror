...
\frame[containsverbatim]{\frametitle{A refinement}
...
\vfill
\textcolor{subtitlex}{Specifying the types protects against changing other
  uses of \texttt{type\_of}.}
}
...
\frame[containsverbatim]{\frametitle{An example: more metavariables}
...
\frame{\frametitle{Future features: {\tt ...} in parameter lists}
...
\frame{\frametitle{Future features: {\tt ...} across control-flow paths}
...
\frame{\frametitle{Future features: Isomorpshisms}
...
\frame{\frametitle{Future features: An isomorpshism for Rust}
For {\tt shutdown\_on\_idle}, the code is always written as:
\begin{lstlisting}[language=tdiff]
let mut e = tokio_executor::enter().unwrap();
e.block_on(rt.shutdown_on_idle());
\end{lstlisting}
...
\frame{\frametitle{Future features: Another isomorpshism for Rust}
...
\begin{itemize}
\item Specify changes one at a time?
